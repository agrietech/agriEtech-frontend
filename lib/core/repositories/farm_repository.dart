...
class FarmRepository {
  ...
  Future<FarmModel> createFarm(CreateFarmRequest request) async {
    try {
      ...
      if ((farmJson['farmName'] == null && farmJson['name'] == null) ||
          farmJson['farmName'] == 'Farm Plot') {
  ...
  /// Fetch planning and performance data using the backend's canonical routes.
  /// Keeping response decoding here prevents screens from relying on a specific
  /// API envelope while the service evolves.
  Future<Map<String, dynamic>> getPlanningData(String path) async {
    try {
  ...
  /// Fetch satellite and soil telemetry for a single farm location.
  Future<Map<String, dynamic>> getHyperLocalTelemetry({
    required double lat,
    required double lng,
    ...
    try {
      final response = await _dioClient.get(
        ...
        queryParameters: {'lat': lat, 'lng': lng, 'crop': crop},
      );
      final raw = response.data;
      if (raw is Map) {
        final data = raw['data'];
