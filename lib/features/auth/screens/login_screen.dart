...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  void _onFieldChanged() {
    if (ref.read(authProvider).error != null ||
        ref.read(authProvider).accountLockoutMessage != null) {
      ref.read(authProvider.notifier).clearError();
  ...
  Future<void> _loginWithPassword([
    ...
    final phone = (phoneOverride ?? _phoneController.text).trim();
    final password = passOverride ?? _passwordController.text;

    if (phoneOverride == null || passOverride == null) {
      if (_formKey.currentState?.validate() != true) return;
    }

    final storage = ref.read(secureStorageServiceProvider);
    if (_rememberMe) {
      await storage.saveRememberedUser(phone);
    } else {
        ...
        const SnackBar(
          content: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Biometric credential not yet linked. Please sign in with password first.',
                ),
              ),
            ],
          ),
